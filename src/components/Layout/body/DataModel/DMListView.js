...
export default function DMListView(props) {
  ...
  const deleteDataRecord = async (data) => {
    //setOpen(true);
    let response;
    response = await Api.getDMDeleteDbConnection(data, authName, authKey);
    response = await Api.getDataModalListView(authName, authKey);
  ...
  async function ViewDataRecord(data) {
    ...
    if (json) {
      ...
      setViewDetailsCreationUser(json.serializer.creationUser);
  ...
  function UpdateItem(item) {
    ...
    setCreateUser(item.creationUser);
    ...
    async function fetchData(deleteRecord) {
      try {
        ...
        if (deleteRecord) {
          ...
          // response = await Api.getDMDeleteDbConnection(id, authName, authKey);
        } else {
          response = await Api.getDataModalListView(authName, authKey);
          ...
          <TableBody>
            {data.map((item) => (
              ...
              >
                ...
                <TableCell>{item.creationUser}</TableCell>
                ...
                <TableCell>
                  <ButtonGroup>
                    <Button
                      ...
                      onClick={() => {
                        history.push({
                          pathname: "/dmdetailsview",
                          state: {
                            data: item, authName, authKey
                          },
                        });
                      }}
                    >
