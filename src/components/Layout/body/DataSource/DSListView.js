...
export default function DSListView(props) {
  ...
  const deleteDataRecord = async (data) => {
    //setOpen(true);
    let response;
    response = await Api.getDSDeleteDbConnection(data, authName, authKey);
    response = await Api.getDataSourceListView(authName, authKey);
  ...
  async function ViewDataRecord(data) {
    let response;
    setIsLoading(true);
    response = await Api.getDSSingleListRecord(data, authName, authKey);
    const json = await response.json();
    if (json) {
      setViewDetails(json);
      setViewDialog(true);
