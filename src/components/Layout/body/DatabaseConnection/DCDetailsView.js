...
export default function DetailedViewPage(props) {
  ...
  const getDataFromServer = async () => {
    const response = await Api.getSingleListRecord(location.state.data.id, authName, authKey);
    const json = await response.json();
    console.log("response inside detailview = ", json);
    setStatus(json["status"]);
    setConfigurationDetail(json["Configuration Details"]);
    setSelectedFileName(json["DB Details"].parameter);
    setParameter(json["DB Details"].parameter);
    setName(json["DB Details"].name);

    setCreationTime(json["DB Details"].creationTime);
    setLastUpdatedBy(json["DB Details"].lastUpdatedUser);
    setLastUpdateTime(json["DB Details"].lastUpdateTime);
    setUser(json["DB Details"].creation_user);
    setCreatorUser(json["DB Details"].creation_user);
    setLoading(false);
  };
  ...
  const saveFile = () => {
    if (name.length !== 0) {
      ...
      updateData(formData, id, authName, authKey);
  ...
  async function updateData(insertRecord, id, authName, authKey) {
    console.log("update is called");
    let response;
    try {
      response = await Api.updateRecordById(insertRecord, id, authName, authKey);
      const json = await response.json();
      getDataFromServer();
      // setData(json);
    } catch (e) {
    ...
    Name
          ...
          <Stack spacing={2} direction="row">
            ...
            <Button
              ...
              size="large"
              onClick={() => {
                Api.getDeleteDbConnection(location.state.data.id, authName, authKey).then(() => {
                  history.push("/");
                });
              }}
            >
